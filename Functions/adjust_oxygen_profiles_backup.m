function cruise_struct = adjust_oxygen_profiles_backup(cruise_struct, brob1,brob2,plot_graph,T)
    ...
    for i = 1:length(subStructNames)
        ...
        %Calculating mean oxygen at these indices
        mean_oxy(ind1) = cruise_struct.(currentSubStruct).CTDOXY_umol_kg_1_ADJ(ind1);
        mean_oxy(ind2) = (mean_oxy(ind2) + cruise_struct.(currentSubStruct).CTDOXY_umol_kg_2_ADJ(ind2))/2;
        ...
        if plot_graph==1
            ...
            legend('Primary sensor', 'Secondary sensor', 'Mean adjusted oxygen','Winkler','Location','northeastoutside')
            ax=gca;
            ax.FontSize=16;
            %ax.XLim=[min(cruise_struct.(currentSubStruct).OXY_MEAN) max(cruise_struct.(currentSubStruct).OXY_MEAN)];
            axis square; grid on
        end
