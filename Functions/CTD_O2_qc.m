...
for ii = 1:length(subStructNames)
    ...
    % Calculate the difference between consecutive values in CTDOXY_umol_kg_1
    % and CTDOXY_umol_kg_2 if they exist.

    if isfield(currentSubStruct, 'CTDOXY_umol_kg_1')
    ...
    %% Seconday sensor, if it exist

    if isfield(currentSubStruct, 'CTDOXY_umol_kg_2')
    ...
    if plot_graph==1
        ...
        if isfield(currentSubStruct, 'CTDOXY_umol_kg_1')
            ...
            plot(currentSubStruct.CTDOXY_umol_kg_1(ix1), ...
                currentSubStruct.CTDPRS(ix1), 'gx','MarkerSize',10,'LineWidth',3);
            %Plot Bad data
            plot(currentSubStruct.CTDOXY_umol_kg_1(ix2), ...
                currentSubStruct.CTDPRS(ix2), 'or','MarkerSize',30,'LineWidth',3);
        end
        ...
        if isfield(currentSubStruct, 'CTDOXY_umol_kg_2')
            ...
            % Plot Questionable data
            plot(currentSubStruct.CTDOXY_umol_kg_2(ix3), ...
                currentSubStruct.CTDPRS(ix3), 'gx','MarkerSize',10,'LineWidth',3);
            %Plot Bad data
            plot(currentSubStruct.CTDOXY_umol_kg_2(ix4), ...
                currentSubStruct.CTDPRS(ix4), 'or','MarkerSize',30,'LineWidth',3);
        end
        ...
        % Initialize the legend labels array
        legend_labels = {};

        % Check for the primary sensor
        if isfield(currentSubStruct, 'CTDOXY_umol_kg_1')
            legend_labels{end+1} = 'Primary sensor O2';
            % Check if there are any questionable or bad data points
            if any(ismember(currentSubStruct.CTDOXY_1_FLAG, 3))
                legend_labels{end+1} = 'Questionable O2 (Primary)';
            end
            if any(ismember(currentSubStruct.CTDOXY_1_FLAG, 4))
                legend_labels{end+1} = 'Bad O2 (Primary)';
            end
        end

        % Check for the secondary sensor
        if isfield(currentSubStruct, 'CTDOXY_umol_kg_2')
            legend_labels{end+1} = 'Secondary sensor O2';
            % Check if there are any questionable or bad data points
            if any(ismember(currentSubStruct.CTDOXY_2_FLAG, 3))
                legend_labels{end+1} = 'Questionable O2 (Secondary)';
            end
            if any(ismember(currentSubStruct.CTDOXY_2_FLAG, 4))
                legend_labels{end+1} = 'Bad O2 (Secondary)';
            end
        end

        % Apply the legend if there are any labels to display
