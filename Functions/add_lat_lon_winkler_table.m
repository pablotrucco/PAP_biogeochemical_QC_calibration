function winkler_table=add_lat_lon_winkler_table(winkler_table,cruise_struct)
    ...
    ctd_casts = winkler_table.CTD;


    % Initialize latitudes and longitudes
    latitudes = NaN(size(winkler_table, 1), 1);
    longitudes = NaN(size(winkler_table, 1), 1);
    ...
    winkler_table.Lat = latitudes;
    winkler_table.Lon = longitudes;

    assignin('base', original_name, winkler_table);
