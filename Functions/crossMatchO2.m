...
% Function to match up Winkler oxygen measurements against the CTD measurements 
% at the bottle closure. This function searches for the corresponding cast and
% corresponding Niskin bottle. Then a robust linear fit is adjusted to 
% this match up, the results are plotted and the coefficients are stored to
% adjust the CTD oxygen profiles.

    Niskin = varargin{1};
    Winkler = varargin{2};
    graphOption = nargin < 3 || varargin{3} == 1;
    ...
    for i = 1:height(Winkler)
        % Find the rows in Niskin where Niskin.CTD is equal to Winkler.CTD and Niskin.Niskin_Bottle is equal to Winkler.Niskin_Bottle
    ...
    if ~isempty(not_found)
        ...
        warning('This row(s) will be not considered in the analysis and remove from the corresponding table. If you want to include them, consider reviewing the original files (.btl and .xls) and the logsheet to find the source of the error')
    end
    ...
    % Calculate in situ density and convert O2 from umol/L to umol/kg
    ...
    % Store the updated Winkler table in the base workspace
...
% Check if the secondary oxygen sensor data is available
hasSecondaryO2 = ismember('Sbeox1ML/L', Niskin.Properties.VariableNames);
...
% Store the updated Niskin table in the base workspace
assignin('base', table1Name, Niskin);

X = Winkler.O2_umol_kg_1;
Y1 = Niskin.primary_O2_umol_kg_1(idx); % For the primary oxygen sensor
Y2 = []; % Initialize Y2 for secondary oxygen sensor

% Prepare for robust linear regression
if hasSecondaryO2
    Y2 = Niskin.secondary_O2_umol_kg_1(idx); % For the secondary oxygen sensor if available
end
...
X1 = X(~exclude_outliers_1);
Y1 = Y1(~exclude_outliers_1);
[brob1, stats1] = robustfit(X1, Y1); % Robust linear regression for primary sensor

%% Secondary oxygen sensor
brob2 = []; % Initialize brob2
stats2 = []; % Initialize stats2

if hasSecondaryO2
    X2 = X(~exclude_outliers_2);
    Y2 = Y2(~exclude_outliers_2);
    [brob2, stats2] = robustfit(X2, Y2); % Robust linear regression for secondary sensor if data is available
end



%% Constructing the output tables
cross_match_table1 = table(Winkler_CTD, Depth, X, Y1, 'VariableNames', {'CTD', 'Depth', 'Winkler_O2', 'Primary_O2'});

if hasSecondaryO2
    cross_match_table2 = table(Winkler_CTD, Depth, X, Y2, 'VariableNames', {'CTD', 'Depth', 'Winkler_O2', 'Secondary_O2'});
else
    cross_match_table2 = []; % or an alternative way to handle missing secondary sensor data
end

%% Graphical analysis and plotting
if graphOption
    ...
    % Conditional plotting for Secondary Oxygen Sensor
    if hasSecondaryO2
