function microcat_table=SAL_TimeSeries_offset_drift_adjust(first_cruise,second_cruise,microcat_table)
...
figure
scatter(microcat_table.Datetime,microcat_table.SalinityPractical_PSU_,20,'filled')
hold on
scatter(tblstats_first.Datetime,tblstats_first.mean_SAL_MEAN,60,'sg','filled','MarkerEdgeColor','k')
...
for ii=1:length(CTD_num_used_first)
    ...
    plot(first_cruise.SAL_MEAN(strcmp(cellstr(first_cruise.CTD_cast),CTD_num_used_first(ii))), ...
        first_cruise.CTDPRS(strcmp(cellstr(first_cruise.CTD_cast),CTD_num_used_first(ii))))
    axis ij
    ax=gca;
    ax.FontSize=14;
    ax.XLabel.String='Salinity';
...
TT_microcat_for_offset=timetable(microcat_table.Datetime(ix_time_cross_first), ...
    microcat_table.SalinityPractical_PSU_(ix_time_cross_first));

TT_CTD_for_offset=timetable(tblstats_first.Datetime,tblstats_first.mean_SAL_MEAN);
...
yl.Label=strcat('Mean offset =', num2str(round(mean(TT_offset.(1)-TT_offset.(2)),2)));
yl.FontSize=14;
yl.FontWeight='bold';

yl2=yline(mean(TT_offset.(1)-TT_offset.(2))+std(TT_offset.(1)-TT_offset.(2)),'Color',clr,'linestyle','--','linewidth',2);
yl2.Label=strcat('1 \sigma',' (',num2str(round(std(TT_offset.(1)-TT_offset.(2)),2)));
yl2.FontSize=14;
yl2.FontWeight='bold';
...
ax=gca;
ax.FontSize=14;
ax.YLabel.String='Sensor offset salinity';
ax.Title.String='Salinity';
box on

%Adjusting the measurement by substracting the offset

microcat_table.SalinityPractical_PSU_Offset_corrected=microcat_table.SalinityPractical_PSU_-(round(mean(TT_offset.(1)-TT_offset.(2)),2));

figure
scatter(microcat_table.Datetime,microcat_table.SalinityPractical_PSU_,20)
hold on
scatter(microcat_table.Datetime,microcat_table.SalinityPractical_PSU_Offset_corrected,20,'*')
scatter(tblstats_first.Datetime,tblstats_first.mean_SAL_MEAN,60,'sg','filled','MarkerEdgeColor','k')
box on
grid on
ax=gca;
...
scatter(microcat_table.Datetime,microcat_table.SalinityPractical_PSU_Offset_corrected,20,'*')
scatter(tblstats_first.Datetime,tblstats_first.mean_SAL_MEAN,60,'sg','filled','MarkerEdgeColor','k')
scatter(tblstats_second.Datetime,tblstats_second.mean_SAL_MEAN,60,'dg','filled','MarkerEdgeColor','k')
box on
grid on
ax=gca;
ax.FontSize=14;
...
for ii=1:length(CTD_num_used_second)
    ...
    plot(second_cruise.SAL_MEAN(strcmp(cellstr(second_cruise.CTD_cast),CTD_num_used_second(ii))), ...
...
TT_microcat_for_linear_drift=timetable(microcat_table.Datetime(ix_time_cross_second), ...
    microcat_table.SalinityPractical_PSU_Offset_corrected(ix_time_cross_second));

TT_CTD_for_linear_drift=timetable(tblstats_second.Datetime,tblstats_second.mean_SAL_MEAN);
...
yl.Label=strcat('Mean offset =', num2str(round(mean(TT_linear_drift.(1)-TT_linear_drift.(2)),2)));
yl.FontSize=14;
yl.FontWeight='bold';
...
yl2.Label=strcat('1 \sigma',' (',num2str(round(std(TT_linear_drift.(1)-TT_linear_drift.(2)),2)));
yl2.FontSize=14;
yl2.FontWeight='bold';
...
ax=gca;
ax.FontSize=14;
ax.YLabel.String='Sensor linear drift final deviation';
ax.Title.String='Salinity';
box on

%Adjusting the measurement by substracting the offset
linear_correction=linspace(0,(round(mean(TT_linear_drift.(1)-TT_linear_drift.(2)),2)),height(microcat_table));
microcat_table.SalinityPractical_PSU_Offset_linear_drift_corrected=microcat_table.SalinityPractical_PSU_Offset_corrected - ...
linear_correction';
...
hold on
scatter(microcat_table.Datetime,microcat_table.SalinityPractical_PSU_Offset_corrected,20,'*')
scatter(microcat_table.Datetime,microcat_table.SalinityPractical_PSU_Offset_linear_drift_corrected,20,'+')
scatter(tblstats_first.Datetime,tblstats_first.mean_SAL_MEAN,60,'sg','filled','MarkerEdgeColor','k')
scatter(tblstats_second.Datetime,tblstats_second.mean_SAL_MEAN,60,'dg','filled','MarkerEdgeColor','k')
